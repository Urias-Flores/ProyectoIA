...
const handSelected = document.querySelector("#hand");
const handPoint = document.querySelector("#hand-points")
...
export class Camera {
  ...
  drawResults(hands) {
    ...
    //Envia  a dibujar los puntos en la imagen de video de cada mano
   ...
   */
   drawKeypoints(keypoints, handedness) {
    ...
    this.ctx.fillStyle = handedness === 'Left' ? '#008F39' : '#E5BE01';
    ...
    if(handSelected.value == handDetected){
      for (const keyPoint of keypointsArray) {
        //Constantes que continen las coordenadas de cada dedo en pantalla
        const y = keyPoint.x;
        const x = keyPoint.y;
      ...
      if(model.value == 2){
        ...
        xLabel.value = "X: "+keypointsArray[handPointSelected].x
        yLabel.value = "Y: "+keypointsArray[handPointSelected].y
      }

      const fingers = Object.keys(fingerLookupIndices);
      for (const currentFinger of fingers) {
  ...
  initProcess(){
    console.log('Me estoy executando')
  }
  ...
  drawKeypoints3D(keypoints, handedness, ctxt) {
    ...
    ctxt.scatterGL.setPointColorer((i) => {
      ...
      return handedness === 'Left' ? '#008F39' : '#E5BE01';
    });
